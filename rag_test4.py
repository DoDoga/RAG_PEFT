...
def get_text_chunks(text):
    text_splitter = RecursivveCharacterTextSplitter(
        chunk_size=900,
        chunk_overlap=100,
...
def main():
    ...
    def print_history():
        for msg in st.session_state.messages:
            st.chat_massage(msg.role).write(msg.content)
            
    def add_history(role, content):
        st.session_state.massages.append(ChatMessage(role=role, content=content))
    ...
    if process:
        ...
        vectorstore = get_vectorstore(files_text)
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = [{"role": "assistant", "content": "안녕하세요 주어진 문서에 대해 궁금하신 것이 있으면 언제나 물어봐주세요!"}]
    ...
    if user_input := st.chat_input("메세지를 입력해 주세요"):
        ...
        st.chat_massages("user").write(f"{user_input}")
        with st.chat_massages("assistant"):
            llm = RemoteRunnable("url 주소 입력")
            chat_container = st.empty
            
            if st.session_state.processsComplete == True:
